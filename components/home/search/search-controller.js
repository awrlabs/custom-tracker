...
trackerCapture.controller('SearchController', function (
        ...
        return new Promise((resolve, reject) => {
            TEIService.customSearch(searchGroup["tJz1lz2sGrl"].value).then(function (res) {
                if (res.status && res.status === 200 && res.data) {
                    ...
                    // unknown state, fall back to normal search
                    reject();
                }
            }).catch(function (err) {
                $scope.searching = null;
                if (err.status && err.status === 460) {
                    ...
                    // unknown error, fall back to normal search
                    reject();
                }
            });
        });
    ...
    $scope.search = function (searchGroup) {
        console.log("Search called", searchGroup);
        if (!$scope.searching) {
            ...
            // do custom search
            return doCustomSearch(searchGroup).then(() => {
                console.log("Custom search was succesful");
                return;
            }).catch(() => {
                console.log("Custom search failed. trying normal search");
                ...
                return promise.then(function (res) {
                    ...
                    if (rowsCnt === 1) {
                        ...
                        return potentialDuplicatesPromise.then(function (duplicates) {
                            $scope.searching = null;
                            console.log("results model", res, duplicates.identifiableObjects ? duplicates.identifiableObjects : [])
                            return showResultModal(res, searchGroup, duplicates.identifiableObjects ? duplicates.identifiableObjects : []);
                        });
                    }
                    $scope.searching = null;
                    console.log("results model", res)
    ...
    var openTei = function (tei, fromAudit) {
        if ($scope.searchingForRelatedTei) {
            ...
            $location.path('/dashboard').search({
                ...
                ou: tei.ou || $scope.selectedOrgUnit.id,
                fromAudit: fromAudit
            });
        }
    }
    ...
    var showResultModal = function (res, searchGroup, existingDuplicates, usingCustomSearch = false) {
        var internalService = {
